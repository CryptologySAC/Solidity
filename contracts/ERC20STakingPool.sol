...
abstract contract ERC20StakingPool is
    IERC20StakingPool,
    ...
    ReentrancyGuard
{
    ...
    uint256 public timestampPoolOpened;
    ...
    function _isStakePoolOpen() internal view returns (bool) {
        ...
        uint256 poolEndTime = timestampPoolOpened + (60 * AVG_SECONDS_PER_MONTH); // 60 months from the opening time
    ...
    function createStake(uint256 _amount, uint8 _tier) external nonReentrant {
        ...
        // check if stake is at lest the minimal amount
    ...
    function unstake(uint256 _stakeID) external nonReentrant {
        ...
        if (
            stake.startTimestamp + (stake.duration * AVG_SECONDS_PER_MONTH) > block.timestamp
        ) {
    ...
    function getAllStakesForUser(address account) external view returns (Stake[] memory) {
